...
B-Impact is the V1 skeleton for a Python repository change-impact analyzer. P2 provides repository registration, SQLite persistence, analysis task lifecycle storage, Python file scanning, AST-based symbol extraction, basic structural/import/inheritance edges, and a Markdown report path without implementing Git diff, change mapping, call graph extraction, scoring, coverage, or test recommendation logic.
...
- Pydantic schemas aligned with ADR 0001, ADR 0002, and ADR 0003
- SQLAlchemy ORM models for `Repository`, `Analysis`, `CodeFile`, `Symbol`, `Edge`, `Impact`, and `TestRecommendation`
- SQLite persistence for repository, analysis, scanned file, symbol, and edge records
...
- Backend tests for health, repository registration, validation errors, analysis creation/querying, report output, symbol extraction, import edges, and local parse failures
...
Local development note: migrations are deferred while the model surface is still changing. If you have a database created before P2, recreate it by removing `backend/data/b-impact.sqlite3` and starting the backend again.
...
The accepted response returns `PENDING`. P2 then scans Python files, extracts symbols and supported edges, records local parse failures, and stores the analysis as `COMPLETED` unless orchestration or persistence fails.
...
The analysis result includes extraction summary fields:
...
Impact-oriented fields remain zero in P2 because diff mapping, impact propagation, scoring, coverage, and test recommendation are not implemented.
...
- Do not add diff, impact, scoring, coverage, or test recommendation logic in P2.
